...
public class NetworkController extends BroadcastReceiver
{
    public static interface SignalCluster
    {
        ...
        public abstract void setNetworkType(NetworkType networktype);
...
_L14:
    ...
    private final void updateDataNetType()
    {
        ...
        if (i >= 0 && i <= 4)
        {
            ...
            } else
            {
                ...
                case 10: // '\n'
                    networktype = NetworkType.Type_3G;
                    if (PluginFactory.getStatusBarPlugin(mContext).get3GPlusResources(mIsRoaming, mDataNetType))
                    {
                        if (!mIsRoaming)
                            mDataIconList = TelephonyIcons.DATA_3G_PLUS[i];
                        else
                            mDataIconList = TelephonyIcons.DATA_3G_PLUS_ROAM[i];
                        mDataTypeIconId = mDataIconList[0];
                        mContentDescriptionDataType = mContext.getString(0x7f0b0095);
                        break;
                    }
                ...
                case 2: // '\002'
                    if (!mShowAtLeastThreeGees)
                    {
                        ...
                        mDataTypeIconId = mDataIconList[0];
                        mContentDescriptionDataType = mContext.getString(0x7f0b0098);
                        break;
                    }
                    // fall through
            ...
            Xlog.d("StatusBar.NetworkController", (new StringBuilder()).append("updateDataNetType, NetworkType=").append(networktype).append(" tempDataTypeIconId= ").append(mDataTypeIconId).append(".").toString());
    ...
    private final void updateTelephonySignalStrength()
    {
        if (!hasService())
        {
            ...
            mPhoneSignalIconId.setIconId(0x7f020211);
            mDataSignalIconId.setResources(null);
            mDataSignalIconId.setIconId(0x7f020211);
        } else
        {
...
label0:
    ...
    void refreshViews()
    {
        ...
        } else
        {
            ...
            if (!mConnected && !flag)
                s1 = context.getString(0x7f0b00a7);
            else
            if (!hasService() && !flag)
                s1 = "";
            else
            ...
            if (mDataConnected && i >= 0 && i <= 3)
            {
                ...
                } else
                {
                    ...
                    {
                    case 3: // '\003'
                        mMobileActivityIconId.setIconId(0x7f02020c);
                        break;
                    ...
                    case 1: // '\001'
                        mMobileActivityIconId.setIconId(0x7f020207);
                        break;
        ...
        if (mWifiConnected)
        {
            if (mWifiSsid == null)
            {
                s2 = context.getString(0x7f0b00a8);
                mWifiActivityIconId = 0;
            } else
            {
                s2 = mWifiSsid;
        ...
        if (mBluetoothTethered)
        {
            ...
            mContentDescriptionCombinedSignal = mContext.getString(0x7f0b009b);
        }
        boolean flag1;
        ...
        {
            mContentDescriptionPhoneSignal = mContext.getString(0x7f0b009c);
            mAirplaneIconId = 0x7f020206;
            mPhoneSignalIconId.setResources(null);
            mPhoneSignalIconId.setIconId(0x7f020206);
            mDataSignalIconId.setResources(null);
            mDataSignalIconId.setIconId(0x7f020206);
            mDataTypeIconId = 0;
            if (mWifiConnected)
            {
                s1 = "";
        ...
        for (Iterator iterator = mSignalClusters.iterator(); iterator.hasNext(); signalcluster.setNetworkType(mNetworkType))
        {
            signalcluster = (SignalCluster)iterator.next();
            signalcluster.setShowSimIndicator(mSimIndicatorFlag, mSimIndicatorResId);
            signalcluster.setRoamingFlagandResource(mIsRoaming, mRoamIconId);
        }
