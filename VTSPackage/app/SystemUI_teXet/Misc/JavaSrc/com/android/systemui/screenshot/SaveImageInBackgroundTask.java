...
class SaveImageInBackgroundTask extends AsyncTask
{
    ...
    SaveImageInBackgroundTask(Context context, SaveImageInBackgroundData saveimageinbackgrounddata, NotificationManager notificationmanager, int i)
    {
        ...
        StringBuilder stringbuilder = (new StringBuilder()).append(resources.getString(0x7f0b0043));
        String s2;
        if (mTickerAddSpace)
            s2 = " ";
        else
            s2 = "";
        mNotificationBuilder = builder.setTicker(stringbuilder.append(s2).toString()).setContentTitle(resources.getString(0x7f0b0044)).setContentText(resources.getString(0x7f0b0045)).setSmallIcon(0x7f020138).setWhen(System.currentTimeMillis());
    ...
    protected void onPostExecute(SaveImageInBackgroundData saveimageinbackgrounddata)
    {
        ...
        } else
        {
            ...
            mNotificationBuilder.setContentTitle(resources.getString(0x7f0b0046)).setContentText(resources.getString(0x7f0b0047)).setContentIntent(PendingIntent.getActivity(saveimageinbackgrounddata.context, 0, intent, 0)).setWhen(System.currentTimeMillis()).setAutoCancel(true);
            Notification notification = mNotificationBuilder.build();
