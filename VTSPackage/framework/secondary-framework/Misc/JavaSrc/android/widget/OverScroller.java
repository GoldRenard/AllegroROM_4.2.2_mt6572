...
label0:
        ...
        boolean update()
        {
            ...
            case 1: // '\001'
                float f7 = (float)l / (float)mDuration;
                float f8 = f7 * f7;
                float f9 = Math.signum(mVelocity);
                d = f9 * (float)mOver * (3F * f8 - f8 * (2.0F * f7));
                mCurrVelocity = 6F * (f9 * (float)mOver) * (f8 + -f7);
                break;

            case 2: // '\002'
                float f6 = (float)l / 1000F;
                mCurrVelocity = (float)mVelocity + f6 * mDeceleration;
                d = f6 * (float)mVelocity + (f6 * (f6 * mDeceleration)) / 2.0F;
                break;
            ...
            case 0: // '\0'
                ...
                if (i < 100)
                {
                    ...
                    f2 = (SPLINE_POSITION[i + 1] - f5) / (f4 - f3);
                    f1 = f5 + f2 * (f - f3);
                }
                d = f1 * (float)mSplineDistance;
                mCurrVelocity = 1000F * ((f2 * (float)mSplineDistance) / (float)mSplineDuration);
                break;
            }
...
*/
        ...
        SplineOverScroller(Context context)
        {
            ...
            mState = 0;
            mFinished = true;
            mPhysicalCoeff = 0.84F * (386.0878F * (160F * context.getResources().getDisplayMetrics().density));
        }
    }
    ...
    public boolean isScrollingInDirection(float f, float f1)
    {
        ...
        return !isFinished() && Math.signum(f) == Math.signum(i) && Math.signum(f1) == Math.signum(j);
    }
