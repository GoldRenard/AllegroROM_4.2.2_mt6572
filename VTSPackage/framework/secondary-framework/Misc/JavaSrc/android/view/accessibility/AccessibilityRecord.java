...
label0:
        {
            AccessibilityRecord accessibilityrecord1;
            synchronized (sPoolLock)
            {
                if (sPool == null)
                    break label0;
                accessibilityrecord1 = sPool;
                sPool = sPool.mNext;
                sPoolSize = -1 + sPoolSize;
                accessibilityrecord1.mNext = null;
                accessibilityrecord1.mIsInPool = false;
            }
    ...
    public AccessibilityNodeInfo getSource()
    {
        ...
        if (mConnectionId != -1 && mSourceWindowId != -1 && AccessibilityNodeInfo.getAccessibilityViewId(mSourceNodeId) != -1)
            return AccessibilityInteractionClient.getInstance().findAccessibilityNodeInfoByAccessibilityId(mConnectionId, mSourceWindowId, mSourceNodeId, 7);
        else
            return null;
    }
